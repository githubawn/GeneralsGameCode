...
public:
	...
	virtual void getOrigin(Int* x, Int* y) override
	{
		*x = m_originX;
		*y = m_originY;
			...
			for (i = 0; i < 4; i++)
			{
				...
				switch (i)
				{
					case 0:
						break;
					case 1:
						name = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel1, &exists);
						break;
					case 2:
						name = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel2, &exists);
						break;
					case 3:
						name = pMapObj->getProperties()->getAsciiString(TheKey_waypointPathLabel3, &exists);
						break;
					default:
						name.clear();
				}
...
{
	if (m_projection)
		return;    // camera doesn't rotate in top down view.
	m_cameraAngle += delta;
	redraw();
	drawLabels();
	CMainFrame::GetMainFrame()->handleCameraChange();
...
{
	if (m_projection)
		return;    // camera doesn't pitch in top down view.
	m_FXPitch += delta;
	redraw();
	drawLabels();
	CMainFrame::GetMainFrame()->handleCameraChange();
