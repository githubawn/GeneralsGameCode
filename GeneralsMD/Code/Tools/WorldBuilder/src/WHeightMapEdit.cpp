...
void WorldHeightMapEdit::reloadTextures()
{
	...
	for (i = 0; i < m_numGlobalTextureClasses; i++)
	{
		if (m_globalTextureClasses[i].numTiles == 0)
			continue;
		CachedFileInputStream stream;
...
Bool WorldHeightMapEdit::optimizeTiles()
{
	...
	for (i = 1; i < m_numBlendedTiles; i++)
	{
		...
		if (blendInfo[i].blendNdx < 0)
			blendInfo[i].blendNdx = 0;
	}
...
Bool WorldHeightMapEdit::doCliffAdjustment(Int xIndex, Int yIndex)
{
	...
	while (pNodesToProcess)
	{
		...
		if (isCliff)
		{
			...
			pProcessed[ndx] = true;
			if (uvRange.lo.x > cliffInfo.u0)
				uvRange.lo.x = cliffInfo.u0;
			if (uvRange.lo.x > cliffInfo.u1)
				uvRange.lo.x = cliffInfo.u1;
			if (uvRange.lo.x > cliffInfo.u2)
				uvRange.lo.x = cliffInfo.u2;
			if (uvRange.lo.x > cliffInfo.u3)
				uvRange.lo.x = cliffInfo.u3;
			if (uvRange.lo.y > cliffInfo.v0)
				uvRange.lo.y = cliffInfo.v0;
			if (uvRange.lo.y > cliffInfo.v1)
				uvRange.lo.y = cliffInfo.v1;
			if (uvRange.lo.y > cliffInfo.v2)
				uvRange.lo.y = cliffInfo.v2;
			if (uvRange.lo.y > cliffInfo.v3)
				uvRange.lo.y = cliffInfo.v3;
			if (uvRange.hi.x < cliffInfo.u0)
				uvRange.hi.x = cliffInfo.u0;
			if (uvRange.hi.x < cliffInfo.u1)
				uvRange.hi.x = cliffInfo.u1;
			if (uvRange.hi.x < cliffInfo.u2)
				uvRange.hi.x = cliffInfo.u2;
			if (uvRange.hi.x < cliffInfo.u3)
				uvRange.hi.x = cliffInfo.u3;
			if (uvRange.hi.y < cliffInfo.v0)
				uvRange.hi.y = cliffInfo.v0;
			if (uvRange.hi.y < cliffInfo.v1)
				uvRange.hi.y = cliffInfo.v1;
			if (uvRange.hi.y < cliffInfo.v2)
				uvRange.hi.y = cliffInfo.v2;
			if (uvRange.hi.y < cliffInfo.v3)
				uvRange.hi.y = cliffInfo.v3;
...
Bool WorldHeightMapEdit::adjustForTiling(TCliffInfo& cliffInfo, Real textureWidth)
{
	...
	if (minU > cliffInfo.u1)
		minU = cliffInfo.u1;
	if (maxU < cliffInfo.u1)
		maxU = cliffInfo.u1;
	if (minU > cliffInfo.u2)
		minU = cliffInfo.u2;
	if (maxU < cliffInfo.u2)
		maxU = cliffInfo.u2;
	if (minU > cliffInfo.u3)
		minU = cliffInfo.u3;
	if (maxU < cliffInfo.u3)
		maxU = cliffInfo.u3;
	...
	if (minU >= uBorder && maxU < textureWidth - uBorder)
		return true;
	if (minU >= -delta && maxU <= textureWidth + delta)
	{
		if (cliffInfo.u0 < uBorder)
			cliffInfo.u0 = 0;
		if (cliffInfo.u1 < uBorder)
			cliffInfo.u1 = 0;
		if (cliffInfo.u2 < uBorder)
			cliffInfo.u2 = 0;
		if (cliffInfo.u3 < uBorder)
			cliffInfo.u3 = 0;
		if (cliffInfo.u0 > textureWidth - uBorder)
			cliffInfo.u0 = textureWidth;
		if (cliffInfo.u1 > textureWidth - uBorder)
			cliffInfo.u1 = textureWidth;
		if (cliffInfo.u2 > textureWidth - uBorder)
			cliffInfo.u2 = textureWidth;
		if (cliffInfo.u3 > textureWidth - uBorder)
			cliffInfo.u3 = textureWidth;
		return true;
	}
	...
	triMaxU = triMinU;
	if (triMinU > cliffInfo.u1)
		triMinU = cliffInfo.u1;
	if (triMaxU < cliffInfo.u1)
		triMaxU = cliffInfo.u1;
	if (triMinU > cliffInfo.u3)
		triMinU = cliffInfo.u3;
	if (triMaxU < cliffInfo.u3)
		triMaxU = cliffInfo.u3;
	...
	triMinU = cliffInfo.u1;
	triMaxU = triMinU;
	if (triMinU > cliffInfo.u2)
		triMinU = cliffInfo.u2;
	if (triMaxU < cliffInfo.u2)
		triMaxU = cliffInfo.u2;
	if (triMinU > cliffInfo.u3)
		triMinU = cliffInfo.u3;
	if (triMaxU < cliffInfo.u3)
		triMaxU = cliffInfo.u3;
...
{
	// return true;
	if (fabs(u1 - u2) < TEX_PER_CELL / 4)
		return true;
	if (u1 == 0 && u2 > 3 * TEX_PER_CELL)
		return true;
	if (u2 == 0 && u1 > 3 * TEX_PER_CELL)
		return true;
	return false;
}
			...
			for (j = yIndex - 1; j < yIndex + 2; j += 1)
			{
				...
				if (m_cliffInfoNdxes[ndx])
				{
					...
					else if (j == yIndex)
					{
						...
						{
							// right
							if (lock1 && !usMatch(tmpCliff.u1, info.u0))
								shifted = true;
							if (lock2 && !usMatch(tmpCliff.u2, info.u3))
								shifted = true;
							if (!shifted)
							{
								tmpCliff.u1 = info.u0;
