	...
	if (obj)
	{
		...
		if (obj->isDisabled())
		{
			if (
			  ...
			  (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED) || obj->isDisabledByType(DISABLED_EMP) || obj->isDisabledByType(DISABLED_SUBDUED)
...
void Drawable::drawDisabled(const IRegion2D* healthBarRegion)
{
	...
	//                   7/
	if (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED) || obj->isDisabledByType(DISABLED_EMP) || obj->isDisabledByType(DISABLED_SUBDUED) || obj->isDisabledByType(DISABLED_UNDERPOWERED))
	{
		// create icon if necessary
		if (getIconInfo()->m_icon[ICON_DISABLED] == nullptr)
		{
			getIconInfo()->m_icon[ICON_DISABLED] = newInstance(Anim2D)(s_animationTemplates[ICON_DISABLED], TheAnim2DCollection);
		}
