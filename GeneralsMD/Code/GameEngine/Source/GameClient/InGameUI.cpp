...
static Bool kindOfUnitSelection(Drawable* test, void* userData)
{
	...
	if (test)
	{
		...
		if (object && isKindOfMatch && object->isLocallyControlled() && !object->isContained() && !object->getDrawable()->isSelected() && !object->isEffectivelyDead() && object->isMassSelectable() && !object->isOffMap())
		{
...
static Bool similarUnitSelection(Drawable* test, void* userData)
{
	...
	if (test)
	{
		...
		// only select objects if not already selected
		if (object && isEquivalent && object->isLocallyControlled() && !object->isContained() && !(object->getDrawable()->isSelected()) && object->isMassSelectable()    // And only if they can be multiply selected. (otherwise the drawable will be, but the object will not be)
		    && !object->isOffMap())
		{
			// enforce optional unit cap
			if (TheInGameUI->getMaxSelectCount() > 0 && TheInGameUI->getSelectCount() >= TheInGameUI->getMaxSelectCount())
	...
	for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it)
	{
		...
		switch (action)
		{
			...
			case ACTIONTYPE_REPAIR_OBJECT:
			{
				...
				success = (TheActionManager->canRepairObject(other->getObject(), objectToInteractWith, CMD_FROM_PLAYER) && (currentRepairer == INVALID_ID || currentRepairer == other->getObject()->getID()));
...
void InGameUI::drawFloatingText()
{
	...
	for (FloatingTextListIt it = m_floatingTextList.begin(); it != m_floatingTextList.end(); ++it)
	{
		...
		if (TheTacticalView->worldToScreen(&ftd->m_pos3D, &pos) && ftd->m_dString && ThePartitionManager->getShroudStatusForPlayer(playerIndex, pCX, pCY) == CELLSHROUD_CLEAR)
		{
