...
SkirmishPreferences::SkirmishPreferences() { loadFromIniFile(); }
...
Bool SkirmishPreferences::loadFromIniFile() {
  if (rts::ClientInstance::getInstanceId() > 1u) {
    AsciiString fname;
    fname.format("Skirmish_Instance%.2u.ini",
...
void reallyDoStart() {
  ...
  if (md) {
    isSkirmish = md->m_isMultiplayer; // we can now select solo campaign maps in
                                      // Skirmish.
  }
  ...
  if (isSkirmish) {
    ...
    msg->appendIntegerArgument(
        DIFFICULTY_NORMAL); // not really used; just specified so we can add the
                            // game speed last
    msg->appendIntegerArgument(
        0); // not really used; just specified so we can add the game speed last
...
static void startPressed() {
  ...
  if (it == TheMapCache->end()) {
    ...
    MessageBoxOk(TheGameText->fetch("GUI:ErrorStartingGame"),
                 TheGameText->fetch("GUI:CantFindMap"), nullptr);
  }
...
else if (playerCount < 2 && !sandboxOk)
{
          sandboxOk = TRUE;
          MessageBoxOk(TheGameText->fetch("GUI:ErrorStartingGame"),
TheGameText->fetch("GUI:SandboxWarning"), nullptr);
}
  */
  else {
    isReady = TRUE;
...
void positionAdditionalImages(MapMetaData *mmd, GameWindow *mapWindow,
  ...
  while (it != mmd->m_supplyPositions.end()) {
    ...
    markerPos.x =
        (position * smallWidth) - SUPPLY_TECH_SIZE / 2 + ul.x; // + winMapPos.x;
  ...
  if (it == TheMapCache->end()) {
    ...
    for (; i < mmd.m_numPlayers && mmd.m_isMultiplayer; ++i) {
      ...
      if (wmIt != mmd.m_waypoints.end()) {
        ...
        DEBUG_CRASH(("positionStartSpots:: someone messed with the map cash.  "
                     "We couldn't find waypoint <%s> in map <%s>",
...
static void handleStartingCashSelection() {
  ...
  if (myGame) {
    ...
    Money startingCash;
    startingCash.deposit(
        (UnsignedInt)GadgetComboBoxGetItemData(comboBoxStartingCash, selIndex),
        FALSE, FALSE);
...
static void handleLimitSuperweaponsClick() {
  ...
  if (myGame) {
    // At the moment, 1 and 0 are the only choices supported in the GUI, though
    // the system could support more.
...
void InitSkirmishGameGadgets() {
  ...
  checkBoxLimitSuperweaponsID = TheNameKeyGenerator->nameToKey(
      "SkirmishGameOptionsMenu.wnd:CheckboxLimitSuperweapons");
  comboBoxStartingCashID = TheNameKeyGenerator->nameToKey(
      "SkirmishGameOptionsMenu.wnd:ComboBoxStartingCash");

  // Initialize the pointers to our gadgets
  parentSkirmishGameOptions = TheWindowManager->winGetWindowFromId(
      nullptr, parentSkirmishGameOptionsID);
  ...
  checkBoxLimitSuperweapons = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, checkBoxLimitSuperweaponsID);
  DEBUG_ASSERTCRASH(checkBoxLimitSuperweapons,
                    ("Could not find the checkBoxLimitSuperweapons"));
  comboBoxStartingCash = TheWindowManager->winGetWindowFromId(
      parentSkirmishGameOptions, comboBoxStartingCashID);
  DEBUG_ASSERTCRASH(comboBoxStartingCash,
                    ("Could not find the comboBoxStartingCash"));
  ...
  textEntryPlayerName =
      TheWindowManager->winGetWindowFromId(nullptr, textEntryPlayerNameID);
  ...
  for (; i < MAX_SLOTS; i++) {
    ...
    GadgetComboBoxGetListBox(comboBoxPlayerTemplate[i])
        ->winSetTooltipFunc(playerTemplateListBoxTooltip);
...
void skirmishUpdateSlotList() {
  if (doUpdateSlotList) {
    ...
    UpdateSlotList(TheSkirmishGameInfo, comboBoxPlayer, comboBoxColor,
                   comboBoxPlayerTemplate, comboBoxTeam, nullptr, buttonStart,
...
static const char *gadgetsToHide[] = {
    "TextEntryPlayerName", "StaticTextPlayers", "StaticTextColor",
    "StaticTextTeam",      "StaticTextFaction", nullptr};
...
void updateSkirmishGameOptions() {
  ...
  GadgetCheckBoxSetChecked(checkBoxLimitSuperweapons,
                           TheSkirmishGameInfo->getSuperweaponRestriction() !=
                               0);
  ...
  DEBUG_ASSERTCRASH(index < itemCount,
                    ("Could not find new starting cash amount %d in list",
                     TheSkirmishGameInfo->getStartingCash().countMoney()));
}
...
void SkirmishGameOptionsMenuInit(WindowLayout *layout, void *userData) {
  ...
  TheSkirmishGameInfo->setSuperweaponRestriction(
      prefs.getSuperweaponRestricted() ? 1 : 0);
...
WindowMsgHandledType SkirmishGameOptionsMenuSystem(GameWindow *window,
  ...
  case GBM_SELECTED: {
    ...
    //				{
    //					static NameKeyType sliderGameSpeedID =
    //TheNameKeyGenerator->nameToKey(
    ...
    //				}
      ...
      skirmishMapSelectLayout =
          TheWindowManager->winCreateLayout("Menus/SkirmishMapSelectMenu.wnd");
...
void populateSkirmishBattleHonors() {
  GameWindow *list = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("SkirmishGameOptionsMenu.wnd:ListboxInfo"));
  if (!list)
    return;
  ...
  GameWindow *streakWindow = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextStreakValue"));
  if (streakWindow) {
  ...
  GameWindow *bestStreakWindow = TheWindowManager->winGetWindowFromId(
      nullptr,
      NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextBestStreakValue"));
  if (bestStreakWindow) {
  ...
  GameWindow *winsWindow = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextWinsValue"));
  if (winsWindow) {
  ...
  GameWindow *lossesWindow = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextLossesValue"));
  if (lossesWindow) {
  ...
  GadgetListBoxAddEntryImage(list, nullptr, 0, 0, 10, 10, TRUE,
  ...
  if (stats.getCHINACampaignComplete(DIFFICULTY_HARD)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("ChinaCampaign_G"),
        TRUE, BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
  } else if (stats.getCHINACampaignComplete(DIFFICULTY_NORMAL)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("ChinaCampaign_S"),
        TRUE, BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
  } else if (stats.getCHINACampaignComplete(DIFFICULTY_EASY)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("ChinaCampaign_B"),
        TRUE, BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("ChinaCampaign_B"),
        (honors & BATTLE_HONOR_CAMPAIGN_CHINA), BATTLE_HONOR_CAMPAIGN_CHINA,
        row, column);
  }
  ...
  if (stats.getGLACampaignComplete(DIFFICULTY_HARD)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("GLACampaign_G"), TRUE,
        BATTLE_HONOR_CAMPAIGN_GLA, row, column);
  } else if (stats.getGLACampaignComplete(DIFFICULTY_NORMAL)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("GLACampaign_S"), TRUE,
        BATTLE_HONOR_CAMPAIGN_GLA, row, column);
  } else if (stats.getGLACampaignComplete(DIFFICULTY_EASY)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("GLACampaign_B"), TRUE,
        BATTLE_HONOR_CAMPAIGN_GLA, row, column);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("GLACampaign_B"),
  ...
  if (stats.getUSACampaignComplete(DIFFICULTY_HARD)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("USACampaign_G"), TRUE,
        BATTLE_HONOR_CAMPAIGN_USA, row, column);
  } else if (stats.getUSACampaignComplete(DIFFICULTY_NORMAL)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("USACampaign_S"), TRUE,
        BATTLE_HONOR_CAMPAIGN_USA, row, column);
  } else if (stats.getUSACampaignComplete(DIFFICULTY_EASY)) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("USACampaign_B"), TRUE,
        BATTLE_HONOR_CAMPAIGN_USA, row, column);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("USACampaign_B"),
  ...
  if (completedOnHard) {
    ...
    InsertBattleHonor(
        ...
        TRUE, BATTLE_HONOR_CHALLENGE_MODE, row, column);
    ...
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Challenge_Bronz"),
        FALSE, BATTLE_HONOR_CHALLENGE_MODE, row, column);
  }
  ...
  InsertBattleHonor(
      ...
      column);
  GadgetListBoxAddEntryImage(list, nullptr, 2, 0, 10, 10, TRUE,
                             GameMakeColor(255, 255, 255, 255));
  ...
  if (!missingBrutal) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("Endurance_G"),
                      TRUE, BATTLE_HONOR_ENDURANCE, row, column);
  } else if (!missingMedium) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("Endurance_S"),
                      TRUE, BATTLE_HONOR_ENDURANCE, row, column);
  } else if (!missingEasy) {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("Endurance_B"),
                      TRUE, BATTLE_HONOR_ENDURANCE, row, column);
  } else {
    InsertBattleHonor(list,
                      TheMappedImageCollection->findImageByName("Endurance_B"),
  ...
  if (streak >= 1000) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_1000"),
        TRUE, BATTLE_HONOR_STREAK, row, column, uStr, streak);
  } else if (streak >= 500) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_500"),
        TRUE, BATTLE_HONOR_STREAK, row, column, uStr, streak);
  } else if (streak >= 100) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("HonorStreak_100"),
        TRUE, BATTLE_HONOR_STREAK, row, column, uStr, streak);
  } else if (streak >= 25) {
    InsertBattleHonor(
  ...
  if (totalWins >= 10000) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Domination_10000"),
        TRUE, BATTLE_HONOR_DOMINATION, row, column, uStr, totalWins);
  } else if (totalWins >= 1000) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Domination_1000"),
        TRUE, BATTLE_HONOR_DOMINATION, row, column, uStr, totalWins);
  } else if (totalWins >= 500) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Domination_500"), TRUE,
        BATTLE_HONOR_DOMINATION, row, column, uStr, totalWins);
  } else if (totalWins >= 100) {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Domination_100"), TRUE,
        BATTLE_HONOR_DOMINATION, row, column, uStr, totalWins);
  } else {
    InsertBattleHonor(
        list, TheMappedImageCollection->findImageByName("Domination_100"),
        FALSE, BATTLE_HONOR_DOMINATION, row, column, uStr, totalWins);
  }
  ...
  for (it = TheMapCache->begin(); it != TheMapCache->end(); ++it) {
    ...
    Int numBrutalOpponentsBeaten =
        stats.getEnduranceMedal(it->first, SLOT_BRUTAL_AI);
    if (numBrutalOpponentsBeaten < totalOpponentSlots) {
      perfect = FALSE;
      break;
    }
  }
  InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Ultimate"),
                    perfect, BATTLE_HONOR_ULTIMATE, row, column);
