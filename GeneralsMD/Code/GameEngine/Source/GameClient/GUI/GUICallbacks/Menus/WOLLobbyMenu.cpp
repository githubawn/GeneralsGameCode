													...
													UnsignedInt mouse)
{
	...
	if (pRoomsInterface != nullptr && pAuthInterface != nullptr && pStatsInterface != nullptr && pSocialInterface != nullptr)
	{
		...
		if (col > 0)
		{
			...
			{
				TheMouse->setCursorTooltip(UnicodeString(L"Error: 1"), -1, nullptr, 1.5f); // the text and width are the only params used.  the others are the default values.
			}
		}
	}
}
		...
		pStatsInterface->findPlayerStatsByBatch(vecUserStatsToRequest, [=](bool bSuccess)
			{
				...
				for (const NetworkRoomMember& netRoomMember : sorted)
				{
					...
					//if (bSuccess)
					{
						...
						numDiscons += GetAdditionalDisconnectsFromUserFile((Int)netRoomMember.user_id);
						...
						pi.m_profileID = (Int)netRoomMember.user_id; // TODO_NGMP: Downcast... we need to use int64_t everywhere really
						pi.m_flags = 0;
					...
					std::set<int>::const_iterator selIt = selectedUserIDs.find((int)netRoomMember.user_id);
		...
		case GCM_SELECTED:
			{
				...
				{
					// TODO_NGMP: Support room re-joining from this combo box again (currently just a filter dropdown)
					/*
					int rowSelected = -1;
