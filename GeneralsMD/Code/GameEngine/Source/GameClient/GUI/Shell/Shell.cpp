...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Shell::Shell() { construct(); }

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Shell::~Shell() { deconstruct(); }
...
void Shell::init() {
  ...
  ini.loadFileDirectory("Data\\INI\\Default\\ShellMenuScheme",
                        INI_LOAD_OVERWRITE, nullptr);
  ini.loadFileDirectory("Data\\INI\\ShellMenuScheme", INI_LOAD_OVERWRITE,
                        nullptr);
...
void Shell::recreateWindowLayouts() {
  ...
  Bool wasClearBackground = m_clearBackground;
  ...
  // Restore preserved active shell state variables
  m_isShellActive = wasShellActive;
  m_shellMapOn = wasShellMapOn;
  m_clearBackground = wasClearBackground;
  m_pendingPush = wasPendingPush;
  m_pendingPop = wasPendingPop;
  m_pendingPushName = wasPendingPushName;

  // restore the screen stack synchronously and cleanly without transition
  // side-effects
  Int screenIndex = 0;
  for (; screenIndex < screenCount; ++screenIndex) {
    ...
    if (newScreen) {
      ...
      // callbacks on these menus causes them to reset/exit the active match.
      // Restrict this to the top-most screen (screenIndex == screenCount - 1)
      // to avoid re-initializing background menus.
      if (m_isShellActive && (screenIndex == screenCount - 1)) {
...
void Shell::push(AsciiString filename, Bool shutdownImmediate) {
  {
    ...
    if (_f) {
      fprintf(_f, "TRACE: Shell::push - Pushing screen: %s\n", filename.str());
      fclose(_f);
    }
  }
  ...
  if (m_screenCount >= MAX_SHELL_STACK) {
    ...
    DEBUG_LOG(("Unable to load screen '%s', max '%d' reached", filename.str(),
               MAX_SHELL_STACK));
    return;
  }
...
void Shell::showShell(Bool runInit) {
  DEBUG_LOG(("Shell:showShell() - %s (%s)", TheGlobalData->m_initialFile.str(),
...
void Shell::doPush(AsciiString layoutFile) {
  ...
  DEBUG_ASSERTCRASH(newScreen != nullptr,
...
void Shell::registerWithAnimateManager(GameWindow *win, AnimTypes animType,
  ...
  if (!m_animateWindowManager) {
    DEBUG_CRASH(("We called registerWithAnimateManager and we don't have an "
                 "Animate Manager created"));
    return;
  }
...
WindowLayout *Shell::getSaveLoadMenuLayout() {
  ...
  if (m_saveLoadMenuLayout == nullptr)
    m_saveLoadMenuLayout =
        TheWindowManager->winCreateLayout("Menus/PopupSaveLoad.wnd");
...
WindowLayout *Shell::getPopupReplayLayout() {
  ...
  if (m_popupReplayLayout == nullptr)
    m_popupReplayLayout =
        TheWindowManager->winCreateLayout("Menus/PopupReplay.wnd");
...
WindowLayout *Shell::getOptionsLayout(Bool create) {
  ...
  if ((m_optionsLayout == nullptr) && (create == TRUE)) {
    m_optionsLayout =
        TheWindowManager->winCreateLayout("Menus/OptionsMenu.wnd");
