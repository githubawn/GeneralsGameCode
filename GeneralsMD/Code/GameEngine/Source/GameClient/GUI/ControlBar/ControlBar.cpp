...
void ControlBar::evaluateContextUI()
{
	...
	if (!TheInGameUI->areSelectedObjectsControllable())
	{
		...
		if (obj->getControllingPlayer() && obj->getControllingPlayer()->getPlayerTemplate() && obj->getControllingPlayer()->getPlayerTemplate()->getBeaconTemplate().compare(obj->getTemplate()->getName()) == 0)
		{
...
void ControlBar::initSpecialPowershortcutBar(Player* player)
{
	...
	if (!player || !pt || !player->isLocalPlayer() || pt->getSpecialPowerShortcutButtonCount() == 0 || pt->getSpecialPowerShortcutWinName().isEmpty() || !player->isPlayerActive())
		return;
...
void ControlBar::populateSpecialPowerShortcut(Player* player)
{
	...
	Int i;
	if (!player || !player->getPlayerTemplate() || !player->isLocalPlayer() || m_currentlyUsedSpecialPowersButtons == 0 || m_specialPowerShortcutButtons == nullptr || m_specialPowerShortcutButtonParents == nullptr)
		return;
	for (i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; ++i)
	{
				...
				if (power->getRequiredScience() != SCIENCE_INVALID)
				{
					...
					else
					{
						...
						if (bestIndex != -1)
						{
							...
							// get command set
							if (!player || !player->getPlayerTemplate() || player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1().isEmpty() || player->getPlayerTemplate()->getPurchaseScienceCommandSetRank3().isEmpty() || player->getPlayerTemplate()->getPurchaseScienceCommandSetRank8().isEmpty())
							{
								continue;
							}
...
{
	if (!m_specialPowerShortcutParent || !m_specialPowerShortcutButtons || !ThePlayerList || !ThePlayerList->getLocalPlayer())
		return;

	const Bool hasValidShortcutButton = canShowSpecialPowerShortcut();

	if (hasValidShortcutButton && m_specialPowerShortcutParent->winIsHidden() && m_contextParent[CP_MASTER] && !m_contextParent[CP_MASTER]->winIsHidden())
	{
		showSpecialPowerShortcut();
		animateSpecialPowerShortcut(TRUE);
	}
...
{
	if (TheScriptEngine->isGameEnding() || !m_specialPowerShortcutParent || !m_specialPowerShortcutButtons || !ThePlayerList || !ThePlayerList->getLocalPlayer())
		return;
	Bool dontAnimate = TRUE;
	for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; ++i)
	{
