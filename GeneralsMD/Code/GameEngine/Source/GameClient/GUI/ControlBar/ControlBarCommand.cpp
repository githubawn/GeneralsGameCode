                                                       ...
                                                       Bool forceDisabledEvaluation) const
{
	...
	if (disabled && BitIsSet(command->getOptions(), IGNORES_UNDERPOWERED) && obj->getDisabledFlags().test(DISABLED_UNDERPOWERED) && obj->getDisabledFlags().count() == 1)
	{
		disabled = false;
	}
		...
		case GUI_COMMAND_FIRE_WEAPON:
		{
			    ...
			    || (w->getPossibleNextShotFrame() == now - 1))
			{
				...
				else
				{
					...
					if (
					  (command->getOptions() & USES_MINE_CLEARING_WEAPONSET) != 0 && !obj->testWeaponSetFlag(WEAPONSET_MINE_CLEARING_DETAIL))
					{
						return COMMAND_AVAILABLE;
					}
