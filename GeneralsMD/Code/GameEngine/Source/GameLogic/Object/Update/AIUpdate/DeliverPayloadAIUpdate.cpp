		...
		else
		{
			...
			if( m_data.m_strafingWeaponSlot != -1 )
			{
				...
				if( velocity->z < 5.0f )
				{
					...
					Real endDiveDistance = WWMath::Sqrtf( endDiveDistanceSquared );
					Real currentDistance = WWMath::Sqrtf( currentDistanceSquared );
...
StateReturnType RecoverFromOffMapState::update() // Success if we should try again, failure if we should give up
{
	...
	Real enterAngle = WWMath::Atan2f(ai->getMoveToPos()->y - enterCoord.y, ai->getMoveToPos()->x - enterCoord.x);
...
StateReturnType HeadOffMapState::onEnter() // Give move order out of town
{
	...
	Real HUGE_DIST = FUDGE * WWMath::Sqrtf(sqr(terrainExtent.hi.x - terrainExtent.lo.x) + sqr(terrainExtent.hi.y - terrainExtent.lo.y));
