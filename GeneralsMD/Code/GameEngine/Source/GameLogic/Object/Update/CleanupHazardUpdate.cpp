...
/// @todo srj use SLEEPY_UPDATE here
	...
	else if( m_moveRange )
	{
		...
		{
			Real fDist = WWMath::Sqrtf( ThePartitionManager->getDistanceSquared( obj, &m_pos, FROM_CENTER_2D ) );
			if( fDist < 25.0f )
			{
...
void CleanupHazardUpdate::fireWhenReady()
{
	...
	if( target && m_moveRange == 0.0f )
	{
		...
		Object *me = getObject();
		Real fDist = WWMath::Sqrtf( ThePartitionManager->getDistanceSquared( me, target, FROM_CENTER_2D ) );
		if( fDist < fireRange )
		{
