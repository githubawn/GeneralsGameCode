...
protected:
	  ...
	  UnsignedInt lifetimeFrames) const
	{
		...
		if (m_diesOnBadLand && obj)
		{
			...
			if (TheTerrainLogic->isUnderwater(riderPos->x, riderPos->y, &waterZ, &terrainZ) && riderPos->z <= waterZ + 10.0f && obj->getLayer() == LAYER_GROUND)
			{
			...
			if (obj->isOffMap() || (cellType == PathfindCell::CELL_CLIFF) || (cellType == PathfindCell::CELL_WATER) || (cellType == PathfindCell::CELL_IMPASSABLE))
			{
