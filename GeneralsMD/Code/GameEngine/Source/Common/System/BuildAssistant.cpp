                                                        ...
                                                        Player* thePlayer)
{
	...
	for (them = iter2->first(); them; them = iter2->next())
	{
		...
		if (them->isKindOf(KINDOF_IMMOBILE))
		{
			...
			if (checkMyExit && checkHisExit && ThePartitionManager->geomCollidesWithGeom(&hisExitPos, hisGeom, them->getOrientation(), &myExitPos, myGeom, angle))
			{
				if (shrouded)
					return LBC_SHROUD;
