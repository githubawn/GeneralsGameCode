...
static const W3dEmitterLinePropertiesStruct _DefaultLineEmitterProps = { 0, 0, 0.0f, 1.5f, 1.0f, 0.0f, 0.0f, 0, 0, 0, 0, 0, 0, 0, 0, 0 };
	...
	switch (tailtype)
	{
		...
		case SAME_AS_HEAD_ALPHA_ZERO:
			...
			else
			{
				...
				if (!TailDiffuse)
					TailDiffuse = NEW_REF(ShareBufferClass<Vector4>, (MaxNum, "ParticleBufferClass::TailDiffuse"));
...
ParticleBufferClass::TailDiffuseTypeEnum ParticleBufferClass::Determine_Tail_Diffuse()
{
	...
	if (shader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_SRC_COLOR)
		return WHITE;
	// Additive
	else if ((shader.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_ONE) && (shader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ONE))
		return BLACK;
	// Screen
	else if ((shader.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_ONE) && (shader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ONE_MINUS_SRC_COLOR))
		return BLACK;
	// Alpha
	else if ((shader.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_SRC_ALPHA) && (shader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ONE_MINUS_SRC_ALPHA))
