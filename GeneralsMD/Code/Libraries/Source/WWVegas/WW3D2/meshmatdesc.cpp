	...
	for (; pass<PassCount; pass++) {
		...
		*/
		...
		if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) && (ColorArray[0] != nullptr) &&
			...
			for (int vidx=0; vidx<VertexCount; vidx++) {
				Vector4 diffuse=DX9Wrapper::Convert_Color(diffuse_array[vidx]);
				Vector4 emissive=DX9Wrapper::Convert_Color(emissive_array[vidx]);
				diffuse.X *= emissive.X;
				diffuse.Y *= emissive.Y;
				diffuse.Z *= emissive.Z;
				diffuse_array[vidx]=DX9Wrapper::Convert_Color(diffuse);
			}
		}
	...
	*/
	for (pass=0; pass<PassCount; pass++) {
		...
		if ( (Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAP) &&
			  (!DX9Wrapper::Is_Initted() || DX9Wrapper::Get_Current_Caps()->Support_Bump_Envmap() == false) )
		{
			kill_pass = true;
		}

		if ( (Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAPLUMINANCE) &&
			  (!DX9Wrapper::Is_Initted() || DX9Wrapper::Get_Current_Caps()->Support_Bump_Envmap_Luminance() == false) )
		{
			kill_pass = true;
		}
		*/
...
{
	if (DX9Wrapper::Is_Initted() && DX9Wrapper::Get_Current_Caps()->Support_NPatches() && WW3D::Get_NPatches_Level()>1) return true;
