...
// TheSuperHackers @refactor bobtista 15/04/2026 Wrap the dynamic shadow volume buffers in
// W3D classes so g_renderBackend draws route through the bgfx capture hooks. Forward declaration.
...
void W3DVolumetricShadowManager::renderShadows( Bool forceStencilFill )
{
	...
	#endif
		// TheSuperHackers @refactor bobtista 15/04/2026 Route stencil + cull state through
		// g_renderBackend so the bgfx capture hooks observe the values the DX8 path applies.
