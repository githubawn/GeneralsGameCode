...
void RoadType::loadTexture(AsciiString path, Int ID)
{
	...
	m_vertexRoad=Create_Vertex_Buffer(WW3D_FVF_XYZDUV1,TheGlobalData->m_maxRoadVertex+4, (s_dynamic?WW3D_USAGE_DYNAMIC:WW3D_USAGE_DEFAULT));
	m_indexRoad=Create_Index_Buffer(TheGlobalData->m_maxRoadIndex+4, (s_dynamic?WW3D_USAGE_DYNAMIC:WW3D_USAGE_DEFAULT));
	m_numRoadVertices=0;
	m_numRoadIndices=0;
...
void W3DRoadBuffer::loadRoadsInVertexAndIndexBuffers()
{
	...
	IndexBufferClass::WriteLockClass lockIdxBuffer(m_roadTypes[m_curRoadType].getIB(), s_dynamic?WW3D_LOCK_DISCARD:0);
	VertexBufferClass::WriteLockClass lockVtxBuffer(m_roadTypes[m_curRoadType].getVB(), s_dynamic?WW3D_LOCK_DISCARD:0);
...
void W3DRoadBuffer::loadLitRoadsInVertexAndIndexBuffers(RefRenderObjListIterator *pDynamicLightsIterator)
{
	...
	UnsignedShort *ib;
	// Lock the buffers.
	IndexBufferClass::WriteLockClass lockIdxBuffer(m_roadTypes[m_curRoadType].getIB());
	VertexBufferClass::WriteLockClass lockVtxBuffer(m_roadTypes[m_curRoadType].getVB());
	vb=(VertexFormatXYZDUV1*)lockVtxBuffer.Get_Vertex_Array();
	ib = lockIdxBuffer.Get_Index_Array();
	// Add to the index buffer & vertex buffer.
