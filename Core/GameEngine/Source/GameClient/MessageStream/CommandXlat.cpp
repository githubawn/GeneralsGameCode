...
void printObjects(Object* obj, void* userData)
{
	...
	AsciiString statusStr = (isDead) ? "Dead" : (isInert) ? "Inert"
	                                                      : "Living";
...
enum
{
	DROPPED_MAX_PARTICLE_COUNT = 1000
};
                                                            ...
                                                            CommandEvaluateType type)
{
	...
	else if (TheInGameUI->areSelectedObjectsControllable() || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_SHORTCUT))
	{
		...
		if (command &&
		    (command->isContextCommand() || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_SHORTCUT))
		{
		...
		else if (command && (command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_CONSTRUCT || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_CONSTRUCT_FROM_SHORTCUT))
		{
...
GameMessageDisposition CommandTranslator::translateGameMessage(const GameMessage* msg)
{
	...
	switch (t)
	{
				...
				if (selectedObject->isLocallyControlled())
				{
					...
					for (temp; temp != selectedDrawable; temp = temp->getNextDrawable())
					{
						...
						if (temp == nullptr)
						{
							...
							if (tempObject && temp->getNextDrawable() == selectedDrawable && !temp->isSelected() && tempObject->isMobile() && tempObject->isLocallyControlled() && !tempObject->isContained() && !tempObject->isKindOf(KINDOF_NO_SELECT))
							{
								newDrawable = temp;
								break;
							}
				...
				if (selectedObject->isLocallyControlled())
				{
					...
					for (temp; temp != selectedDrawable; temp = temp->getNextDrawable())
					{
						...
						if (temp == nullptr)
						{
							...
							const Object* tempObject = temp->getObject();
							// must take case of this case here or else the loop will break without getting newDrawable
							if (tempObject && temp->getNextDrawable() == selectedDrawable && !temp->isSelected() && tempObject->isMobile() && tempObject->isLocallyControlled() && !tempObject->isContained())
							{
								newDrawable = temp;
								break;
							}
						...
						{
							const Object* tempObject = temp->getObject();
							if (tempObject && !temp->isSelected() && tempObject->isMobile() && tempObject->isLocallyControlled() && !tempObject->isContained() && tempObject->isKindOf(KINDOF_DOZER))
							{
								newDrawable = temp;
								break;
							}
		...
		*/
		...
		{
			const CommandButton* command = TheInGameUI->getGUICommand();
			if (TheInGameUI->getSelectCount() > 0 || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_SHORTCUT))    // If something is selected
			{
		...
		case GameMessage::MSG_MOUSE_RIGHT_CLICK:
		{
			...
			if (TheGlobalData->m_useAlternateMouse && TheMouse->isClick(&m_mouseRightDragAnchor, &m_mouseRightDragLift, m_mouseRightDown, m_mouseRightUp))
			{
				...
				const CommandButton* command = TheInGameUI->getGUICommand();
				Bool controllable = TheInGameUI->areSelectedObjectsControllable() || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_SHORTCUT);
		...
		case GameMessage::MSG_META_DEBUG_CYCLE_EXTENT_TYPE:
			if (extentModType == EXTENTMOD_INVALID)
				extentModType = EXTENTMOD_TYPE;
			if (!extentModAmount)
				extentModAmount = 1.0f;
			FALLTHROUGH;
		case GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MAJOR:
			if (extentModType == EXTENTMOD_INVALID)
				extentModType = EXTENTMOD_MAJOR;
			if (!extentModAmount)
				extentModAmount = 1.0f;
			FALLTHROUGH;
		case GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_MAJOR_BIG:
			if (extentModType == EXTENTMOD_INVALID)
				extentModType = EXTENTMOD_MAJOR;
			if (!extentModAmount)
				extentModAmount = EXTENT_BIG_CHANGE;
			FALLTHROUGH;
		...
		case GameMessage::MSG_META_DEBUG_INCREASE_EXTENT_HEIGHT:
			if (extentModType == EXTENTMOD_INVALID)
				extentModType = EXTENTMOD_HEIGHT;
			if (!extentModAmount)
				extentModAmount = 1.0f;
			FALLTHROUGH;
