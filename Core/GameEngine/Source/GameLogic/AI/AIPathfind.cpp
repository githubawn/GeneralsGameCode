...
)
{
	...
	if (closeNode && closeNode->getNextOptimized())
	{
		...
		Real offsetDist = (offsetDistSq <= 0.0) ? 0.0 : WWMath::Sqrtf(offsetDistSq);
...
#define NO_REAL_DIST
#ifdef REAL_DIST
	Int cost = COST_ORTHOGONAL*WWMath::Sqrtf(dx*dx + dy*dy);
#else
	if (dx<0) dx = -dx;
	if (dy<0) dy = -dy;
	Int cost;
...
UnsignedInt PathfindCell::costToHierGoal( PathfindCell *goal )
{
	...
	Int dx = m_info->m_pos.x - goal->getXIndex();
	Int dy = m_info->m_pos.y - goal->getYIndex();
	Int cost = REAL_TO_INT_FLOOR(COST_ORTHOGONAL*WWMath::Sqrtf(dx*dx + dy*dy) + 0.5f);
	return cost;
}
		...
		for( int i=0; i<numNeighbors; i++ )
		{
			...
			if (goalCell) {
				if (attackDistance == NO_ATTACK)  {
					...
					costRemaining = COST_ORTHOGONAL*WWMath::Sqrtf(dx*dx + dy*dy);
													 ...
													 const Coord3D *rawTo)
{
	...
	if (TheGlobalData->m_debugAI == AI_DEBUG_PATHS)
	{
		...
		Int count = WWMath::Sqrtf(dx*dx+dy*dy)/(PATHFIND_CELL_SIZE_F/2);
		if (count<2) count = 2;
		Int i;
		color.green = 0;
		for (i=1; i<count; i++) {
