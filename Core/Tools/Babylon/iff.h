...
#define IFF_READ(iff, data, size, label) \
	{ \
		if (IFF_Read((iff), (data), (size)) != (size)) \
			goto label; \
	}
#define IFF_WRITE(iff, data, size, label) \
	{ \
		if (IFF_Write((iff), (data), (size)) != (size)) \
			goto label; \
	}
#define IFF_NEWCHUNK(iff, id, label) \
	{ \
		if (!IFF_NewChunk((iff), (id))) \
			goto label; \
	}
#define IFF_NEWFORM(iff, id, label) \
	{ \
		if (!IFF_NewForm((iff), (id))) \
			goto label; \
	}
