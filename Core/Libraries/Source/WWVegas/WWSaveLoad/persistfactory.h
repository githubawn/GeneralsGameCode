...
{
	// TheSuperHackers @bugfix githubawn 19/06/2026 The saved object "pointer" is a
	// pointer-sized identity used for load-time pointer remapping; write it at
	// sizeof(pointer) so it stays symmetric with the Load side, which reads
	// sizeof(T*). The old (uint32)obj cast lost the high bits of a 64-bit pointer
	// AND only matched the Load size when uint32 was (wrongly) 8 bytes on LP64.
