...
RingRenderObjClass::RingRenderObjClass()
  ...
  , IsAnimating(false)
  , LODBias(1.0f)
  , CurrentLOD(RING_NUM_LOD)
  ,    // RING_NUM_LOD does not include the null LOD
  AnimDuration(0.0F)
...
void RingRenderObjClass::Set_Name(const char* name)
{
	...
	(void)nameLen;
	WWASSERT(nameLen < ARRAY_SIZE(Name));
}
