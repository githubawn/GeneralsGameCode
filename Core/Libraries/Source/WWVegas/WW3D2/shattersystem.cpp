...
)
{
	...
	for (int ipass=0; ipass<v0.PassCount; ipass++) {
		Vector4 dcg_v0=DX9Wrapper::Convert_Color(v0.DCG[ipass]);
		Vector4 dcg_v1=DX9Wrapper::Convert_Color(v1.DCG[ipass]);
		Vector4 dig_v0=DX9Wrapper::Convert_Color(v0.DIG[ipass]);
		Vector4 dig_v1=DX9Wrapper::Convert_Color(v1.DIG[ipass]);
					...
					for (ipass=0; ipass<mtl_params.PassCount; ipass++) {
						...
						*/
						...
						if (mtl_params.DIG[ipass] != nullptr) {
							...
							Vector4 mc=DX9Wrapper::Convert_Color(mycolor);
							Vector4 dc=DX9Wrapper::Convert_Color(vert.DIG[ipass]);
							mc=Vector4(mc.X*dc.X,mc.Y*dc.Y,mc.Z*dc.Z,mc.W);
							mycolor=DX9Wrapper::Convert_Color(mc);
						}
