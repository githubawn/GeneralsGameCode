...
void Get_WW3D_ZFormat_Name(WW3DZFormat format, StringClass& name)
{
	switch (format)
	{
		...
		case WW3D_ZFORMAT_D16_LOCKABLE:
			name = "D16Lockable";
			break;    // 16-bit z-buffer bit depth. This is an application-lockable surface format.
		case WW3D_ZFORMAT_D32:
			name = "D32";
			break;    // 32-bit z-buffer bit depth.
		case WW3D_ZFORMAT_D15S1:
			name = "D15S1";
			break;    // 16-bit z-buffer bit depth where 15 bits are reserved for the depth channel and 1 bit is reserved for the stencil channel.
		case WW3D_ZFORMAT_D24S8:
			name = "D24S8";
			break;    // 32-bit z-buffer bit depth using 24 bits for the depth channel and 8 bits for the stencil channel.
		case WW3D_ZFORMAT_D16:
			name = "D16";
			break;    // 16-bit z-buffer bit depth.
		case WW3D_ZFORMAT_D24X8:
			name = "D24X8";
			break;    // 32-bit z-buffer bit depth using 24 bits for the depth channel.
		case WW3D_ZFORMAT_D24X4S4:
			name = "D24X4S4";
			break;    // 32-bit z-buffer bit depth using 24 bits for the depth channel and 4 bits for the stencil channel.
	}
}
