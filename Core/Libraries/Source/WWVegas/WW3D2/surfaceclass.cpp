	...
	SurfaceFormat(format)
{
	WWASSERT(width);
	WWASSERT(height);
	D3DSurface = DX9Wrapper::_Create_DX9_Surface(width, height, format);
}

SurfaceClass::SurfaceClass(const char *filename):
	D3DSurface(nullptr)
{
	D3DSurface = DX9Wrapper::_Create_DX9_Surface(filename);
...
SurfaceClass::LockedSurfacePtr SurfaceClass::Lock(int *pitch)
{
	...
	DX9_ErrorCode(D3DSurface->LockRect(&lock_rect, nullptr, 0));
...
unsigned char *SurfaceClass::CreateCopy(int *width,int *height,int*size,bool flip)
{
	...
	*size=mysize;
	...
	D3DLOCKED_RECT lock_rect;
	::ZeroMemory(&lock_rect, sizeof(D3DLOCKED_RECT));
	DX9_ErrorCode(D3DSurface->LockRect(&lock_rect,nullptr,D3DLOCK_READONLY));
	unsigned int i;
	unsigned char *mem=(unsigned char *) lock_rect.pBits;
	...
	const SurfaceClass *other)
{
	...
	if (sd.Format==osd.Format && sd.Width==osd.Width && sd.Height==osd.Height)
	{
		...
		dst.x=dstx;
		dst.y=dsty;
		DX9Wrapper::_Copy_DX9_Rects(other->D3DSurface,&src,1,D3DSurface,&dst);
	}
	else
	{
		RECT dest;
		...
		DX9_ErrorCode(D3DXLoadSurfaceFromSurface(D3DSurface,nullptr,&dest,other->D3DSurface,nullptr,&src,D3DX_FILTER_NONE,0));
	}
}
	...
	const SurfaceClass *other)
{
	...
	DX9_ErrorCode(D3DXLoadSurfaceFromSurface(D3DSurface,nullptr,&dest,other->D3DSurface,nullptr,&src,D3DX_FILTER_TRIANGLE ,0));
}
