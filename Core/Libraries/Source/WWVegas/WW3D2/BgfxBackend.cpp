...
// TheSuperHackers @refactor bobtista 10/04/2026 BgfxBackend stub.
//
...
// and linked against WW3D2. Later work fills in real implementations as
...
// link error early rather than deep inside the port.
...
// the earlier per-preset shader pairs.
...
// TheSuperHackers @refactor bobtista 11/04/2026 Tracks
...
// TheSuperHackers @refactor bobtista 16/04/2026 bgfx now renders
...
// TheSuperHackers @refactor bobtista 16/04/2026 bgfx resolution
// matches the main game window. Set during Initialize from GetClientRect.
int g_bgfxWidth  = 800;
int g_bgfxHeight = 600;

// TheSuperHackers @refactor bobtista 11/04/2026 program handle
...
// TheSuperHackers @refactor bobtista 14/04/2026 tree / grass
...
// CSM caster pass program + render target.
...
// vertex layouts. One per common FVF format
// FVF table). Initialized in Initialize, used by the vertex
...
// at write-lock time. See the write-side capture comments below.
...
// actual sun direction — hooked up with the CSM work.
...
// TheSuperHackers @refactor bobtista 16/04/2026 Create a secondary
...
void BgfxBackend::Initialize(void * hwnd, int /*width*/, int /*height*/)
{
    ...
    // TheSuperHackers @refactor bobtista 11/04/2026 force
    ...
    // Uber shader program — single program for all engine draws.
    ...
    // Tree/grass sway vertex shader program. Reuses fs_uber.
    ...
    // stencil shadow volume program. XYZ-only verts, no color output.
    ...
    // shadow darken apply program. Fullscreen clip-space quad.
    ...
    // CSM caster program + shadow map render target.
    ...
    // TheSuperHackers @feature bobtista 16/04/2026 Create a DX8
...
void BgfxBackend::Shutdown()
{
    ...
    // The bgfx window is the main game window, do not destroy it.
    g_bgfxWindow = nullptr;
...
void BgfxBackend::End_Scene(bool /*flip_frame*/)
{
    ...
    // The shadow map (view 8) renders FIRST so the depth
...
void BgfxBackend::Set_Vertex_Buffer(const VertexBufferClass * vb, unsigned int stream)
{
    ...
    // The cache is populated by Capture_Vertex_Data on the engine's
                                     ...
                                     unsigned short vertex_count)
{
    ...
    // CSM: also submit sorted geometry as shadow caster.
...
namespace
{
// TheSuperHackers @refactor bobtista 11/04/2026 actual bgfx
                      ...
                      unsigned short vertex_count)
{
    ...
    // CSM: compute light view-projection once for both
    ...
    // The tree / grass sway shader takes over the program slot
    ...
    // CSM caster pass. Submit opaque and sort-flush geometry
