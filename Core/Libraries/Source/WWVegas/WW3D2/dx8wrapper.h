...
WWINLINE void DX8_ErrorCode(unsigned res)
{
	...
	#define DX8_THREAD_ASSERT() \
		...
		{ \
			WWASSERT_PRINT(DX8Wrapper::_Get_Main_Thread_ID() == ThreadClass::_Get_Current_Thread_ID(), "DX8Wrapper::DX8 calls must be called from the main thread!"); \
		}
#else
	#define DX8CALL_HRES(x, res) \
		res = DX8Wrapper::_Get_D3D_Device8()->x; \
	...
	switch ((int)transform)
	{
		case D3DTS_WORLD:
			if (render_state_changed & WORLD_IDENTITY)
				...
				m = To_Matrix4x4(render_state.world);
			break;
		case D3DTS_VIEW:
			if (render_state_changed & VIEW_IDENTITY)
				m.Make_Identity();
			else
				m = To_Matrix4x4(render_state.view);
			break;
		default:
			D3DMATRIX dxm;
