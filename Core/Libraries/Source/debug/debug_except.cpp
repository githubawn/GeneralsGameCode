...
static BOOL CALLBACK ExceptionDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	SendDlgItemMessage(hWnd, 105, WM_SETFONT, (WPARAM)CreateFont(13, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, nullptr), MAKELPARAM(TRUE, 0));

	// exception type
	SendDlgItemMessage(hWnd, 100, WM_SETTEXT, 0, (LPARAM)DebugExceptionhandler::GetExceptionType(exPtrs, regInfo));
...
LONG __stdcall DebugExceptionhandler::ExceptionFilter(struct _EXCEPTION_POINTERS* pExPtrs)
{
	...
	dbg << GetExceptionType(pExPtrs, regInfo) << ":\n"
	    << regInfo << "\n\n";
	LogExceptionLocation(dbg, pExPtrs);
	dbg << "\n\n";
