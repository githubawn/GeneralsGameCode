...
DebugIOFlat::DebugIOFlat()
  ...
  , m_lastStreamPtr(&m_firstStream)
  , m_lastSplitPtr(&m_firstSplit)
{
	*m_copyDir = 0;
}
                          ...
                          unsigned argn, const char* const* argv)
{
	if (!cmd || strcmp(cmd, "help") == 0)
	{
		if (!argn)
			...
			dbg << "splitadd <types> <filter> <name> [ <size in kb> ]\n\n"
	...
	else if (strcmp(cmd, "splitadd") == 0)
	{
		...
		if (argn >= 3)
		{
			...
			for (const char* p = argv[0]; *p; ++p)
			{
				...
				{
					case 'a':
						cur->stringTypes |= 1 << Assert;
						break;
					case 'c':
						cur->stringTypes |= 1 << Check;
						break;
					case 'l':
						cur->stringTypes |= 1 << Log;
						break;
					case 'h':
						cur->stringTypes |= 1 << Crash;
						break;
					case 'x':
						cur->stringTypes |= 1 << Exception;
						break;
					case 'r':
						cur->stringTypes |= 1 << CmdReply;
						break;
					case 'o':
						cur->stringTypes |= 1 << Other;
						break;
				}
			}
			if (!cur->stringTypes)
