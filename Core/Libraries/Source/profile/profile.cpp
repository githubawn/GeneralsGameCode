...
static ProfileCmdInterface& cmd = *(ProfileCmdInterface*)new (ProfileAllocMemory(sizeof(ProfileCmdInterface))) ProfileCmdInterface();
...
void Profile::StopRange(const char* range)
{
	...
	DFAIL_IF(k == m_names)
	return;
	DFAIL_IF(!m_frameNames[k].isRecording)
	return;
