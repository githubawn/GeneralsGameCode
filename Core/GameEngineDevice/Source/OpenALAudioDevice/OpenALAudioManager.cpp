...
{
	if (m_playingSounds.size() >= m_num2DSamples)
		return 0;
	return m_num2DSamples - (UnsignedInt)m_playingSounds.size();
}

//-------------------------------------------------------------------------------------------------
UnsignedInt OpenALAudioManager::getNumAvailable3DSamples(void) const
{
	if (m_playing3DSounds.size() >= m_num3DSamples)
		return 0;
	return m_num3DSamples - (UnsignedInt)m_playing3DSounds.size();
}
