...
void W3DTerrainBackground::doPartialUpdate(const IRegion2D& partialRange, WorldHeightMap* htMap, Bool doTextures)
{
	...
	if (partialRange.lo.x > maxX)
		return;
	if (partialRange.lo.y > maxY)
		return;
	if (partialRange.hi.x < minX)
		return;
	if (partialRange.hi.y < minY)
		return;
	...
	for (j = minY; j <= maxY; j += STEP)
	{
		...
		{
			if (m_curNumTerrainVertices >= m_vertexTerrainSize)
				return;
		...
		*curIb++ = width - 1;
		...
		for (j = 0; j < height; j++)
		{
			*curIb++ = j * yOffset + yOffset + width - 1;
			...
			for (i = width - 2; i >= 0; i--)
			{
				if (m_curNumTerrainIndices + 2 > m_indexTerrainSize)
					return;
				*curIb++ = j * yOffset + i;
