...
#include "WW3D2/dx8fvf.h" // for VertexFormatXYZDUV2
...
void W3DTerrainBackground::doPartialUpdate(const IRegion2D &partialRange, WorldHeightMap *htMap, Bool doTextures )
{
	...
	if (m_vertexTerrainSize<requiredVertexSize || m_vertexTerrain==nullptr) {
		...
		m_vertexTerrain=Create_Vertex_Buffer(WW3D_FVF_XYZDUV1,m_vertexTerrainSize+4,WW3D_USAGE_DEFAULT);
	}
	...
	if (m_indexTerrainSize<requiredIndexSize || m_indexTerrain==nullptr) {
		...
		REF_PTR_RELEASE(m_indexTerrain);
		m_indexTerrain=Create_Index_Buffer(m_indexTerrainSize+4,WW3D_USAGE_DEFAULT);
	}
	Int minX = m_xOrigin;
	Int minY = m_yOrigin;
...
void W3DTerrainBackground::doTesselatedUpdate(const IRegion2D &partialRange, WorldHeightMap *htMap, Bool doTextures )
{
	...
	if (m_vertexTerrainSize<requiredVertex || m_vertexTerrain==nullptr) {
		...
		REF_PTR_RELEASE(m_vertexTerrain);
		m_vertexTerrain=Create_Vertex_Buffer(WW3D_FVF_XYZDUV2,m_vertexTerrainSize+4,WW3D_USAGE_DEFAULT);
	}
...
void W3DTerrainBackground::drawVisiblePolys(RenderInfoClass & rinfo, Bool disableTextures)
{
	...
	g_renderBackend->Set_Vertex_Buffer(m_vertexTerrain,0);
  if (!disableTextures) {
