...
Bool ScreenDefaultFilter::postRender(FilterModes mode, Coord2D &scrollDelta,Bool &doExtraRender)
{
	...
	DX8Wrapper::_Draw_Primitive_UP(D3DPT_TRIANGLESTRIP, 2, v, sizeof(_TRANS_LIT_TEX_VERTEX));
...
//			color.z = 1.0f;
		...
		DX8Wrapper::_Set_Pixel_Shader_Constant(1,   &color, 1);
		D3DXVECTOR4 fadeColor(m_curFadeValue, m_curFadeValue, m_curFadeValue, 1.0f);
		DX8Wrapper::_Set_Pixel_Shader_Constant(2,	&fadeColor, 1);
...
Int ScreenBWFilter::shutdown()
{
	if (m_dwBWPixelShader)
		DX8Wrapper::_Delete_Pixel_Shader(m_dwBWPixelShader);
...
Bool ScreenBWFilterDOT3::postRender(FilterModes mode, Coord2D &scrollDelta,Bool &doExtraRender)
{
	...
	ShaderClass::Invalidate();	//reset DOT3 blend from frame
	ShaderClass shader=ShaderClass::_PresetAlphaShader;
	shader.Set_Depth_Compare(ShaderClass::PASS_ALWAYS);
	DX8Wrapper::Set_Shader(shader);
	DX8Wrapper::Apply_Render_State_Changes();	//force
...
Int TerrainShaderPixelShader::shutdown()
{
	if (m_dwBasePixelShader)
		DX8Wrapper::_Delete_Pixel_Shader(m_dwBasePixelShader);

	if (m_dwBaseNoise1PixelShader)
		DX8Wrapper::_Delete_Pixel_Shader(m_dwBaseNoise1PixelShader);

	if (m_dwBaseNoise2PixelShader)
		DX8Wrapper::_Delete_Pixel_Shader(m_dwBaseNoise2PixelShader);
...
void W3DShaderManager::init()
{
	...
	if ((res=W3DShaderManager::getChipset()) != 0)
	{
		...
		m_oldRenderSurface = DX8Wrapper::_Get_DX8_Render_Target();
		if (m_oldRenderSurface) m_oldRenderSurface->AddRef();
		HRESULT hr = (m_oldRenderSurface != nullptr) ? S_OK : E_FAIL;
		...
		{
			m_renderTexture = DX8Wrapper::_Create_DX8_Texture(desc.Width,desc.Height,D3DFormat_To_WW3DFormat(desc.Format), MIP_LEVELS_1, D3DPOOL_DEFAULT, true);
		...
		if (hr != S_OK)
		{
			...
			else if (W3DShaderManager::getChipset() >= DC_GEFORCE3)
			{
				m_oldDepthSurface = DX8Wrapper::_Get_DX8_Depth_Stencil_Surface();
				if (m_oldDepthSurface) m_oldDepthSurface->AddRef();
				hr = (m_oldDepthSurface != nullptr) ? S_OK : E_FAIL;
...
void W3DShaderManager::startRenderToTexture()
{
	...
	DX8Wrapper::Set_Render_Target(m_newRenderSurface,m_oldDepthSurface);
	HRESULT hr = S_OK;
...
IDirect3DTexture8 *W3DShaderManager::endRenderToTexture()
{
	...
	DX8Wrapper::Set_Render_Target(m_oldRenderSurface,m_oldDepthSurface);	//restore original render target
	HRESULT hr = S_OK;
	DEBUG_ASSERTCRASH(hr==S_OK, ("Set target failed unexpectedly."));
	if (hr == S_OK)
	{
...
HRESULT W3DShaderManager::LoadAndCreateD3DShader(const char* strFilePath, const DWORD* pDeclaration, DWORD Usage, Bool ShaderType, DWORD* pHandle)
{
	...
	try
	{
		...
		{
			hr = DX8Wrapper::_Create_Vertex_Shader(pDeclaration, pShader, pHandle, Usage);
		}
		else // SHADERTYPE_PIXEL
		{
			hr = DX8Wrapper::_Create_Pixel_Shader(pShader, pHandle);
		}
...
{
	if (m_dwBasePixelShader)
		DX8Wrapper::_DeletePixelShader(m_dwBasePixelShader);

	if (m_dwBase0PixelShader)
		DX8Wrapper::_DeletePixelShader(m_dwBase0PixelShader);

	if (m_dwBaseNoise1PixelShader)
		DX8Wrapper::_DeletePixelShader(m_dwBaseNoise1PixelShader);

	if (m_dwBaseNoise2PixelShader)
		DX8Wrapper::_DeletePixelShader(m_dwBaseNoise2PixelShader);
