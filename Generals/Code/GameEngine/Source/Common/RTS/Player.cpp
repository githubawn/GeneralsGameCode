...
void Player::initFromDict(const Dict* d)
{
	...
	if (skirmish)
	{
		...
		else
		{
			...
			AsciiString qualifier;
			qualifier.format("%d", m_mpStartIndex);
			ScriptList* scripts = TheSidesList->getSkirmishSideInfo(skirmishNdx)->getScriptList()->duplicateAndQualify(qualifier, qualTemplatePlayerName, pname);
			...
			for (i = 0; i < TheSidesList->getNumSkirmishTeams(); i++)
			{
				if (TheSidesList->getSkirmishTeamInfo(i)->getDict()->getAsciiString(TheKey_teamOwner) == originalPlayerName)
				{
					...
					if (TheSidesList->findTeamInfo(newName))
						continue;
...
void Player::becomingTeamMember(Object* obj, Bool yes)
{
	...
	if (obj->isKindOf(KINDOF_DOZER) && obj->getAIUpdateInterface() && obj->getAIUpdateInterface()->isIdle())
	{
...
static void doFindCommandCenter(Object* obj, void* userData)
{
	...
	if (info->obj == nullptr && obj->isKindOf(KINDOF_COMMANDCENTER) && obj->getTemplate()->getDefaultOwningSide() == info->player->getSide() && !obj->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) && !obj->testStatus(OBJECT_STATUS_SOLD))
	{
		info->obj = obj;
	}
}
...
static void doFindSpecialPowerSourceObject(Object* obj, void* userData)
{
	...
	if (!obj->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) && !obj->testStatus(OBJECT_STATUS_SOLD) && !obj->isEffectivelyDead())
	{
...
void Player::transferAssetsFromThat(Player* that)
{
	...
	for (Upgrade* upgrade = that->m_upgradeList; upgrade; upgrade = upgrade->friend_getNext())
	{
		...
		if (upgrade->getStatus() == UPGRADE_STATUS_IN_PRODUCTION && upgradeTemplate->getUpgradeType() == UPGRADE_TYPE_PLAYER && (hasUpgradeComplete(upgradeTemplate) || hasUpgradeInProduction(upgradeTemplate)))
		{
...
{
	return (
	  !TheVictoryConditions->hasSinglePlayerBeenDefeated(this) && !m_isPlayerDead && !TheInGameUI->isClientQuiet() && TheGameLogic->isInGameLogicUpdate() && TheGameLogic->getFrame() > 0);
}
