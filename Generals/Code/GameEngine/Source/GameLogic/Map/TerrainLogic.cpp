...
{
	if (pLoc->x < m_bounds.lo.x)
		return (false);
	if (pLoc->x > m_bounds.hi.x)
		return (false);
	if (pLoc->y < m_bounds.lo.y)
		return (false);
	if (pLoc->y > m_bounds.hi.y)
		return (false);
...
void Bridge::updateDamageState()
{
	...
	if (m_bridgeInfo.bridgeObjectID == 0)
		return;
	Object* bridge = TheGameLogic->findObjectByID(m_bridgeInfo.bridgeObjectID);
	if (bridge)
	{
...
Bool TerrainLogic::isPurposeOfPath(Waypoint* pWay, AsciiString label)
{
	...
	Bool match = false;
	if (label == pWay->getPathLabel1())
		match = true;
	if (label == pWay->getPathLabel2())
		match = true;
	if (label == pWay->getPathLabel3())
		match = true;
...
void TerrainLogic::flattenTerrain(Object* obj)
{
	...
	switch (obj->getGeometryInfo().getGeomType())
	{
		case GEOMETRY_BOX:
		{
			...
			if (rawDataHeight > centerHeight)
				rawDataHeight = centerHeight;
