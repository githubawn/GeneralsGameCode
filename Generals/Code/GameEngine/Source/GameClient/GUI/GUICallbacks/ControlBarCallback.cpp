	...
	switch (msg)
	{
		...
		*/
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			if (command && (command->getCommandType() == GUI_COMMAND_SPECIAL_POWER || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER) && BitIsSet(command->getOptions(), NEED_TARGET_POS))
				targeting = TRUE;
		...
		case GWM_LEFT_DOWN:
		{
			    ...
			    TheRadar->radarToWorld(&radar, &world))
			{
				...
				if (drawableList->empty() || (!TheGlobalData->m_useAlternateMouse && msg == GWM_RIGHT_DOWN) || (TheGlobalData->m_useAlternateMouse && msg == GWM_LEFT_DOWN))
				{
