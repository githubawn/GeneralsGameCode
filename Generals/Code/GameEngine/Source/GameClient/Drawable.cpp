...
Bool Drawable::getShouldAnimate(Bool considerPower) const
{
	...
	if (obj)
	{
		...
		if (obj->isDisabled())
		{
			if (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED) || obj->isDisabledByType(DISABLED_EMP)
...
void Drawable::drawDisabled(const IRegion2D* healthBarRegion)
{
	...
	//                   7/
	if (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED) || obj->isDisabledByType(DISABLED_EMP) || obj->isDisabledByType(DISABLED_UNDERPOWERED))
	{
		// create icon if necessary
