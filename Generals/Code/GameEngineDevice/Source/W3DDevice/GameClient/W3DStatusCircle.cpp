...
void W3DStatusCircle::Render(RenderInfoClass & rinfo)
{
	...
	if( TheGlobalData->m_showTeamDot )
	{
		...
		// TheSuperHackers @refactor bobtista 10/04/2026 First IRenderBackend call site
		...
		// does not expose the low-level D3D8 state API yet.
	...
	switch (fade) {
		...
		case ScriptEngine::FADE_SUBTRACT:
			// Low-level blend op not yet abstracted by IRenderBackend (D3DRS_* skipped).
